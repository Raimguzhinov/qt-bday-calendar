...
int main(int argc, char *argv[]) {
  ...
  QObject::connect(
      ...
      [&w, &friends_id, &bdates, &fios, &photos, oauth]() {
        const QUrl getFriends{"https://api.vk.com/method/"
                              "friends.get?fields=bdate,photo_200_orig"};
        auto network_reply = oauth->post(getFriends, {{"v", 5.131}});
        QObject::connect(
            network_reply, &QNetworkReply::finished,
            [&w, &friends_id, &bdates, &fios, &photos, network_reply]() {
              ...
              for (const auto &user_id :
                   response["response"]["items"].toArray()) {
                if (user_id.toObject()["bdate"].toString() != "") {
                  QString tmp_date_resolver =
                      user_id.toObject()["bdate"].toString();
                  ...
                  photos.push_back(
                      user_id.toObject()["photo_200_orig"].toString());
